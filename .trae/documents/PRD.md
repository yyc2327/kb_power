...
|------|----------|
| 搜索框 | 智能搜索输入，支持自然语言查询 |
| 全文搜索 | 基于关键词的精确匹配，支持模糊搜索 |
| 语义搜索 | 理解查询意图，支持同义词、相关概念扩展 |
| 混合检索引擎 | 融合全文搜索和语义搜索的结果 |
| 过滤器面板 | 按知识类型、时间、状态、知识域筛选 |
| 搜索结果 | 高亮匹配词条、显示相关度评分 |
| 知识卡片 | 预览知识摘要，支持快速收藏 |
| 查询历史 | 记录最近搜索，支持一键重搜 |
| 智能推荐 | 基于搜索行为推荐相关知识 |
...
**混合检索算法**：
1. 将用户查询同时发送给全文索引和向量索引
2. 全文检索返回关键词匹配结果（BM25 算法）
3. 向量检索返回语义相似结果（余弦相似度）
4. 使用 Reciprocal Rank Fusion 合并两个结果集
5. 综合评分 = α × 全文得分 + (1-α) × 向量得分
6. α 参数可根据查询类型动态调整
